...
Apple's web presence is a masterclass in **reverent product photography framed by near-invisible UI**. Every page is a stack of edge-to-edge product "tiles" — alternating light and dark canvases, each centered on a hero headline, a one-line tagline, two tiny red pill CTAs, and an impossibly crisp product render. Nothing competes with the product. Typography is confident but quiet; color is either pure white, an off-white parchment, or a near-black tile; interactive elements are a single, decisive KT Real Red.

Density is unusually low even by contemporary SaaS standards. Each tile occupies roughly one viewport, and there is no decorative chrome — no borders, no gradients, no decorative frames, no shadows on headlines. Elevation appears only when a product image rests on a surface (a single soft `rgba(0, 0, 0, 0.22) 3px 5px 30px` drop for visual weight). The result is a catalog that feels more like a museum gallery: the wall disappears and the artifact takes over.

Store and shop surfaces retain the same chassis but switch modes. The product configurator (iPhone 17 Pro, accessories grid) introduces a tight grid of white utility cards at `{rounded.lg}` (18px) radius with a thin border, paired with a persistent thin sub-nav strip. The environment page leans darker and more editorial. Across all five surfaces the typographic system, spacing rhythm, and the single red accent are consistent — this is one design language expressed at different volumes.

**Key Characteristics:**
- Photography-first presentation; UI recedes so the product can speak.
- Alternating full-bleed tile sections: white/parchment ↔ near-black, with the color change itself acting as the section divider.
- Single red accent (`{colors.primary}` — #E60012, KT Real Red) carries every interactive element. No second brand color exists.
- Two button grammars: tiny red pill CTAs (`{rounded.pill}`) and compact utility rects (`{rounded.sm}`).
...
> **KT 브랜드 컬러 시스템 적용.** 컬러 토큰만 KT(한국통신) 브랜드 가이드 기준으로 치환했고, 그 외 모든 타이포·레이아웃·컴포넌트 규칙은 원본 디자인 언어를 그대로 따른다. KT 브랜드 코어는 단일 강조색 **KT Real Red**(#E60012)와 **KT Energy Black**(#1B1B1B), 그리고 따뜻한 톤의 오프화이트로 구성된다.

### Brand & Accent
- **KT Real Red** (`{colors.primary}` — #E60012): The single brand-level interactive color. All text links, all red pill CTAs ("자세히 보기", "가입하기"), and the focus ring root. This is KT's unmistakable "click me" signal. Press state shifts via the active scale transform rather than a hex change.
- **KT Focus Red** (`{colors.primary-focus}` — #FF1A2C): A marginally brighter sibling of Real Red, reserved for the keyboard focus ring on buttons (`outline: 2px solid`).
- **KT Coral on Dark** (`{colors.primary-on-dark}` — #FF5A6A): A brighter coral-red used on dark surfaces for in-copy links and inline callouts, where Real Red would muddy against the near-black tile background.

### Surface
- **Pure White** (`{colors.canvas}` — #ffffff): The dominant canvas. Content, utility cards, store tiles, configurator grids.
- **Warm Parchment** (`{colors.canvas-parchment}` — #F4F4F6): The signature KT off-white — slightly cooler than ivory, used for alternating light tiles, footer region, and the default page canvas in store utility sections. Just different enough from white to create rhythm.
- **Pearl Button** (`{colors.surface-pearl}` — #FAFAFB): A near-white used as the fill for secondary "ghost" buttons — lighter than the parchment canvas so the button still reads as a button against `{colors.canvas-parchment}`.
- **Energy Black Tile 1** (`{colors.surface-tile-1}` — #1B1B1B): KT Energy Black — the primary dark-tile surface on the homepage product grid.
- **Energy Black Tile 2** (`{colors.surface-tile-2}` — #232323): A micro-step lighter — used where a dark tile sits directly above or below Tile 1 to create the faintest separation.
- **Energy Black Tile 3** (`{colors.surface-tile-3}` — #161616): A micro-step darker — used at the bottom of the stack and in embedded video/player frames.
...
- **Translucent Chip Gray** (`{colors.surface-chip-translucent}` — #D4D4D8): The base hex of the translucent gray chip used over photography for circular control buttons. In production, applied at ~64% alpha as `rgba(212, 212, 216, 0.64)`.

### Text
- **Energy Ink** (`{colors.ink}` — #1B1B1B): The voice of every headline, every body paragraph, and the dark utility button's fill. KT Energy Black, chosen instead of pure black to keep the page feeling photographic rather than printed.
- **Body** (`{colors.body}` — #1B1B1B): Same hex as ink — one near-black tone for all text on light surfaces.
...
- **Ink Muted 80** (`{colors.ink-muted-80}` — #2E2E2E): Body text on the white Pearl Button surface — slightly softer than pure black.
- **Ink Muted 48** (`{colors.ink-muted-48}` — #767676): Disabled button text and legal fine-print. KT Cool Gray 600.

### Hairlines & Borders
- **Divider Soft** (`{colors.divider-soft}` — #EFEFF1): The "border" tone on secondary buttons — functions as a ring shadow rather than a hard line. In production, often applied as `rgba(0, 0, 0, 0.04)`.
- **Hairline** (`{colors.hairline}` — #DEDEE1): The 1px hairline border on store utility cards and configurator chips. KT Cool Gray 200.

### Brand Gradient
**No decorative gradients.** KT's brand language—like Apple's—relies on product imagery and the single Real Red accent rather than CSS gradient overlays. Atmospheric depth comes from photography (5G antenna fields, fiber-optic light trails, urban skylines at dusk). No gradient tokens are defined; the strict red/black/white triad does the work.
...
| `{rounded.pill}` | 9999px | Primary red pill CTAs, sub-nav buy button, configurator option chips, search input — the signature pill |
...
**`button-primary`** — The signature KT action. Background `{colors.primary}` (KT Real Red #E60012), text `{colors.on-primary}` in `{typography.body}` (SF Pro Text 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. The full-pill radius IS the brand action signal.
- Active state: `{component.button-primary-active}` — `transform: scale(0.95)` (the system-wide micro-interaction).
- Focus state: `{component.button-primary-focus}` — 2px solid `{colors.primary-focus}` outline.

**`button-secondary-pill`** — Used as the second CTA when two red pills appear together ("자세히 보기" / "가입하기"). Background transparent, text `{colors.primary}`, 1px solid `{colors.primary}` border, rounded `{rounded.pill}`, padding 11px × 22px. Reads as a "ghost pill."

**`button-dark-utility`** — Global nav actions (로그인, 장바구니, 언어 선택). Background `{colors.ink}` (#1B1B1B — KT Energy Black), text `{colors.on-dark}` in `{typography.button-utility}` (14px / 400 / -0.224px tracking), rounded `{rounded.sm}` (8px), padding 8px × 15px. Active state shrinks via `transform: scale(0.95)`.

**`button-pearl-capsule`** — Product-card secondary button. Background `{colors.surface-pearl}` (#FAFAFB), text `{colors.ink-muted-80}` in `{typography.caption}` (14px), 3px solid `{colors.divider-soft}` border (functions as a soft ring rather than a visible line), rounded `{rounded.md}` (11px), padding 8px × 14px.

**`button-store-hero`** — A larger primary CTA used on store hero surfaces. Same KT Real Red + Paper White as `{component.button-primary}`, but with `{typography.button-large}` (18px / 300 — note the rare weight 300) and slightly more padding (14px × 28px). Used sparingly on the store landing.
...
**`text-link`** — Inline body links in `{colors.primary}` (KT Real Red). Underlined or non-underlined per context.

**`text-link-on-dark`** — Inline body links on dark tiles in `{colors.primary-on-dark}` (KT Coral on Dark #FF5A6A) — Real Red would lose punch against `{colors.surface-tile-1}`.
...
**`product-tile-parchment`** — Same as `{component.product-tile-light}` but on `{colors.canvas-parchment}` (#F4F4F6). Used to break two consecutive white tiles.

**`product-tile-dark`** — Full-bleed dark tile. Background `{colors.surface-tile-1}` (#1B1B1B — KT Energy Black), text `{colors.on-dark}`, rounded `{rounded.none}`, vertical padding `{spacing.section}` (80px). Same content stack as the light tile but with `{component.text-link-on-dark}` for inline copy and `{component.button-primary}` (KT Real Red holds up against the dark surface). Used on the homepage product grid as the alternating dark band.

**`product-tile-dark-2`** — Variant on `{colors.surface-tile-2}` (#232323). Used where a dark tile sits directly above or below `{component.product-tile-dark}` to create the faintest separation through micro-step lightness change.

**`product-tile-dark-3`** — Variant on `{colors.surface-tile-3}` (#161616). Used at the bottom of the stack and in embedded video/player frames.
...
**`environment-quote-card`** — A photographic-canvas hero specific to the brand-story page. Dark photographic backdrop (fiber-optic light trails / city at dusk) with `{colors.surface-tile-1}` as the fallback color, centered white-text headline in `{typography.display-lg}` (40px), small KT Real Red pictographic logo above the headline, single `{component.button-primary}` below. Padding `{spacing.section}` (80px).
...
**`footer`** — Background `{colors.canvas-parchment}` (#F4F4F6), text `{colors.ink-muted-80}`. Link columns in `{typography.dense-link}` (17px / 400 / 2.41 line-height — the relaxed leading is what makes the dense columns scannable). Column headings in `{typography.caption-strong}` (14px / 600). Legal row at the very bottom in `{typography.fine-print}` (12px / 400) with `{colors.ink-muted-48}` text. Vertical padding 64px.

## Do's and Don'ts

### Do
- Use `{colors.primary}` (KT Real Red #E60012) for every interactive element — links, pill CTAs, focus signals — and nothing else. The single accent is non-negotiable.
...
- Reserve `{rounded.pill}` for the primary red CTA and any other element that should read as an "action" (configurator chips, search input, sticky bar CTA).
...
### Don't
- Don't introduce a second accent color; every "click me" signal is `{colors.primary}` (KT Real Red).
...
- Don't use `{colors.primary-on-dark}` (KT Coral on Dark) on light surfaces — it's the dark-tile-only variant. Real Red is for light surfaces.
